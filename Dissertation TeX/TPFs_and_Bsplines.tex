...
\end{array} \right.\]
\item \label{eq:dd_property_9} Let $\tau = \left\{\tau_i \right\}}$, $i=1,\dots,n$ be a nondecreasing sequence, and let $g$ be a ``smooth enough'' function. Define \emph{the restriction of $g$ to the sequence $\tau$} to be the vector $\g \vert_\tau = \left(g_1,\dots,g_n\right)^T$, where
\[
