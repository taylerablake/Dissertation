...
\subsection{More B-spline Properties}
...
Hastie and Tibshirani were the first to introduce the varying coefficient model; consider data of the form 
\[
\left(x_i, y_i, t_i\right), \qquad i=1,\dots,m
\]
where $y_i$ is the response, $x_i$ is the single (univariate) regressor variable, and $t_i$ is an indexing variable. The single regressor varying-coefficient (VC) model extends the classical linear model by allowing the slope coefficient to vary smoothly in the dimension of the indexing variable, $t$. The VC model assumes that the mean response is of the form
\[
E\left[ Y\left(t\right) \right] = f\left(t\right)x\left(t\right)
\]
where $f\left(t\right)$ is the smooth slope function of interest. This modeling approach permits interpolation $x$ and $y$ at values of the indexing variable $t$ at which there is either missing data of only a single observation and slope estimation is not feasible. In the section that follows, we will discuss the approach to smoothing the coefficient vector (and \emph{not} the regression function $x\left(t\right)$) first, for mechanical demonstration of parameterization and estimation of the coefficient function via B-spline basis expansion, at a predetermined set of values of $t$ (knots), then following the approach of Eilers and Marx by assuming that the number and position of the knots are unknown and using penalized B-splines, or P-splines 
