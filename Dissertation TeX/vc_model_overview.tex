...
\]
\noindent
with $M \rightarrow \infty$. There are numerous choices for the particular type of basis functions such as logarithms, power functions, or wavelets; there is, however, disadvantages to using basis functions with unrestricted support. Piecewise polynomials and splines are families of functions, each member of which having bounded support, so as to allow for local representations of $m\left(x\right)$, though remain very easy to implement, as their estimation is carried out through the global optimization of a loss function. These methods in the classical setting have been explored extensively; Chen (2007) provides an extensive review of the asymptotic theory of these estimators. Zhou, Shen, and Wolfe (1998) establish asymptotic normality of regression splines; they present explicit expressions for the asymptotic pointwise bias and variance of the estimator, providing a method of constructing confidence intervals and confidence regions.
