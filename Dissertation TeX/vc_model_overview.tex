...
The indexing of B-splines needs some care, especially when we are going to use derivatives. The indexing connects a B-spline to a knot; that is, it gives the index of the knot that characterizes the position of the B-spline. Our choice is to take the leftmost knot, the knot at which the B-spline starts to be- come nonzero. Let $B_j\left(x; q\right)$ denote the value at $x$ of the $j^{th}$ B-spline of degree $q$ for a given equidistant grid of knots. A fitted curve 5 to data (xi,y,) is the linear combination (x) = C5=lcijBj(x; q). When the degree of the B-splines is clear from the context, or immaterial, we use Bj(x) instead of Bj(x; q).
...
Tensor product B-splines exist in the v × v ? plane. For our presentation, n (n ?) equally- spaced indexing knots are placed on v (v ?) to yield a regularly-spaced grid, carving out the plane into subrectangles. The rth-sth single tensor product Br(v)B ?s(v ?), as presented in Figure 4, is positive in the rectangular region defined by the knots R = [?r, ?r+q+2] × [? ?s, ? ?s+q ?+2] or on a support of spanned by (q + 2) × (q ? + 2) knots. Similar to univariate B-splines, it is convenient to index each tensor product by one of the n×n ? knot pairs and

Figure~\ref{fig:bicubic_bspline} displays the tensor product of the two univariate cubic B-splines, say, $B_j$ and $B_{j^*}$ presented along the axes, say $x$ and $\breve{x}$, respectively. Tensor product As with the $x$ axis, using equally-spaced knots $\breve{\phi}$, a similar division of the $\breve{x}$ axis may be constructed to index $B_{j^*}\left(\breve{x} \right)$ for $j^* = 1, \dots , n^*$, possibly using a different $q$. The tensor product value is zero in the corresponding zero univariate $x$ and $\breve{x}$ ranges. More details follow below, including indexing, but
