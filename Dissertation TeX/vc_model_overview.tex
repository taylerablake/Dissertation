...
Tensor product B-splines exist in the $x \times \tilde{x}$ plane. For our presentation, $n$ ($\tilde{n}$) equally-spaced indexing knots are placed on $x$ ($\tilde{x}$) to yield a regularly-spaced grid, carving out the plane into sub-rectangles. The $r^{th}-s^{th}$ single tensor product $B_r\left(x\right)$ ($B_s\left(\tilde{x}\right)$), as presented in Figure~\ref{fig:bicubic_bspline} and Figure~\ref{fig:bicubic_bspline_contour}, is positive in the rectangular region defined by the knots $R = \left[\phi_r, \phi_{r+q+2}\right] \times \left[\tilde{\phi}_s, \tilde{\phi}_{s+\tilde{q}+2}\right]$ or on a support of spanned by $\left(q + 2\right) \times \left(\tilde{q} + 2\right)$ knots. Similar to univariate B-splines, it is convenient to index each tensor product by one of the $n\times\tilde{n}$ knot pairs, with

\begin{equation} \nonumber
\begin{split}
B_r\left(\tilde{x}\right)B_s\left(\tilde{x}\right) & > 0 \;\; \textup{for all }  x,\; \tilde{x}\;\ \in R\\
 & = 0 \;\; \textup{for all }  x,\; \tilde{x}\;\ \not\in R
\end{split}
\end{equation}
\noindent
for $r = 1,dots, n$, $s = 1,dots, \tilde{n}$. Figure~\ref{fig:sparse_bicubic_basis} displays nine tensor product B-splines, which represents only a portion of a full basis. A graphic of a complete basis would be difficult to appreciate, as the``hills'' overlap quite a lot, making each individual basis function difficult to isolate from the rest. Associated with each ``hill'' in Figure~\ref{fig:sparse_bicubic_basis} , there is an unknown coefficient. {\needsparaphrased A complete tensor product B-splines basis thus has an unknown coefficient matrix, denoted by $\Gamma_{n\times\tilde{n}} = \left[\gamma_{rs}\right]. For given knot grid, a very flexible surface can be approximated, e.g. at the digitized coordinates. For $j = 1 ,\dots,p$ and $k = 1,\dots, \tilde{p}$,

\begin{equation} 
\alpha\left(x_{j}, \tilde{x}_{k}\right) = \sum_{r=1}^n \sum_{s=1}^\tilde{n} B_r\left(x_{j}\right) B_s\left(\tilde{x}_{k}\right) \gamma_{rs}
\end{equation}
...
%Figure~\ref{fig:bicubic_bspline} displays the tensor product of the two univariate cubic B-splines, say, $B_j$ and $B_{j^*}$ presented along the axes, say $x$ and $\tilde{x}$, respectively. %Tensor product As with the $x$ axis, using equally-spaced knots $\tilde{\phi}$, a similar division of the $\tilde{x}$ axis may be constructed to index $B_{j^*}\left(\tilde{x} \right)$ for $j^* = 1, %\dots , n^*$, possibly using a different $q$. Similarly to the univariate case, we also have that 

\begin{figure}[H]
  \centering
  \graphicspath{{img/}}
  \includegraphics[width=4in,height=4in]{sparse_bicubic_basis.png}
  \caption{Landscape of cubic B-splines, a portion of a full bivariate basis}\label{sparse_bicubic_basis}
\end{figure}
...
\begin{equation}\textup{vec}\left\{\alpha\left(v, \tilde{v}\right)\right\} = T^*\gamma
\end{equation}

 where $\gamma = \textup{vec}\left(\Gamma\right)$. Define the matrix
