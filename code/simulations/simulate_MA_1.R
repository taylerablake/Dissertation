...
for (i in 1:nrow(Sigma_MA_1)) {
      for (j in 1:ncol(Sigma_MA_1)) {
            if (abs(i-j)==1)
            Sigma_MA_1[i,j] <- sig2*theta
      }
}
...
matplot(t(mvrnorm(n=20,mu=rep(0,m),Sigma=Sigma_MA_1)),col="pink",type="l")

bPars <- rbind(c(0,1,50,3,100,2),
               c(0,1,60,3,100,3))


y <- mvrnorm(n=N,mu=rep(0,m),Sigma=Sigma_MA_1)
...
save(ma_1_coef_list,file="stationary_ar1_coef_list.Rdata")
...
ma1_phi_list <- lapply(ma_1_coef_list,function(beta){
      B_lm%*%beta
})
i <- 1
...
ma_1_coef_list_2 <- list.zip(lam_l=lambdas$lam_m,
                           lam_m=lambdas$lam_l) %>%
      lapply(.,function(l){
...
setwd("/Users/taylerblake/Documents/Dissertation/code/simulations")
save(ma_1_coef_list,file="stationary_ma1_coef_list_dm_1_dl_3.Rdata")
knot_grid <- subset(knot_grid,keep==TRUE)




data.frame(expand.grid(s=1:M,t=1:M),
           phi=as.vector(diag(rep(1,nrow(T_mod)))-T_mod)) %>%
      subset(.,t<s) %>%
      orderBy(~t+s,.) %>%
      wireframe(phi~t+s,
