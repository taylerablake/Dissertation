# Computer lab 2, Bilabo course

# Load supporting functions
setwd(file.path("/Users","taylerblake","Documents","Dissertation","code","Marx Pspline Course"))
source('bases.r')
source('ps_normal.r')
source('ps_poisson.r')
...
pn2 = ps.normal(x, y, bdeg = 3, nseg = 40, lambda = 10, pord = 2, se = 2)
# Decorate the graph
...
for (k in 1:length(lla)) {
      lambda = 10 ^ lla[k]
      pp =  pb1 = ps.binomial(x, y, nseg = 20, lambda = lambda, plot = F)
      aics[k] = pp$aic
}
plot(lla, aics, xlab = 'log10(lambda)', ylab = 'CV')
lines(lla, aics)

best.lambda <- 10^lla[which.min(aics)]
pb1 = ps.binomial(x, y,lambda=best.lambda,nseg=20)
title('Kyphosis data', xlab = 'Age', ylab = 'Occurence')
