...
"psp2dM"<-
      function(y, M, p1, p2, M.type = "stacked", M1.index = NULL, M2.index = NULL, 
               Pars, ridge.adj = 0, M.pred = M, y.predicted = NULL, x.lab = "X1", 
                     ...
                     NULL, r.gamma = NULL, int = F)
      {
            ...
            parms <- pspline2d.checker(family, link, Pars[1, 4], Pars[2, 4], Pars[1,6],
            ...
            ps.fit <- pspline.fitter(family, link, n.col, m.binomial, r.gamma, y,
                                     b = Q, Pen, p.ridge, nix = c(z1, z2),
                                     nix.ridge = rep(0, n1 * n2), ridge.adj, wts)
            mu <- ps.fit$mu
            pcoef <- ps.fit$coef
                  ...
                  if(se.bands) {
                        ...
                        if(image.plot) {
                              image(M2.index[in2], M1.index[in1], t(U.hatm[
                                    in1, in2]), xlab = x.lab, ylab = y.lab,
                                    sub = "2 se Upper Surface")
                        }
                        persp(M2.index[in2], M1.index[in1], t(L.hatm[in1, in2]),
                              xlab = x.lab, ylab = y.lab,
                              zlab = "2 se Lower Surface")
                        if(image.plot) {
                              image(M2.index[in2], M1.index[in1], t(L.hatm[
                                    in1, in2]), xlab = x.lab, ylab = y.lab,
                                    sub = "2 se Lower Surface")
                        }
                  }
            }
            ...
            if(!missing(M.pred)) {
                  ...
                  if(link == "logit") {
                        ...
                        if(!missing(y.predicted)) {
                              ...
                              bin.percent.correct <- (count1 + count2)/length(y.predicted)
                              bin.1percent <- count1/sum(y.predicted)
                              bin.0percent <- count2/(length(y.predicted) - sum(y.predicted))
                        }
                  }
                  if(link == "probit") {
                  ...
                  dimnames(summary.predicted) <- list(NULL,
                                                      c("-2std_Lower", "Predicted", "+2std_Upper"))
            }
            P <- list(coef = pcoef, Pars = Pars,
                      yint = yint,
                      int = int, family, 
                      link, dev = dev, aic = aic,
                      bin.percent.correct =  bin.percent.correct,
                      bin.0 = bin.0percent, bin.1 = bin.1percent,
                      df.resid = m - trace, dispersion.parm = dispersion.parm,
                      mu = mu, press.mu = press.mu, summary.predicted = summary.predicted, 
...
"bsplbase"<-
      function(x, bpars)
      {
            ...
            knots <- seq(bpars[1] - bpars[4] * dx, bpars[2] + bpars[4] * dx, by = 
                               dx)
            base <- as.matrix(spline.des(knots, x, bpars[4] + 1, 0 * x,outer.ok = TRUE)$design)
            base
      }
"pspline2d.checker"<- function(family, link,
                               degree1, degree2,
                               order1, order2,
                               ps.intervals1,
                               ps.intervals2,
...
"pspline.fitter"<- function(family, link, n.col, m.binomial,
                            r.gamma, y, b, p, p.ridge,
                            nix, nix.ridge, ridge.adj, wts, ...){
            ...
            repeat {
                  ...
                  u <- (y - mu)/h.prime + eta
                  if(ridge.adj > 0) {
                        f <- lsfit(rbind(b, p, p.ridge),
                                   c(u, nix, nix.ridge),
                                   wt = c(wts, nix + 1, nix.ridge + 1) *c(w, (nix +1), (nix.ridge + 1)),
                                   intercept = F)
                  }
                  if(ridge.adj == 0) {
                        f <- lsfit(rbind(b, p), c(u, nix), wt = c(wts, nix + 1)*c(w, (nix + 1)), intercept = F)
                  }
                  coef.old <- coef.est
