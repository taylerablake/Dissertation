...
knotGrid <- merge(knotGrid,
      data.frame(l=knots.l,Bl.index = c(1:ncol(Bl),rep(ncol(Bl),length(knots.l)-ncol(Bl)))),
      by="l") %>%
      merge(.,data.frame(m=knots.m,Bm.index = c(1:ncol(Bm),rep(ncol(Bm),length(knots.m)-ncol(Bm)))),
            by="m")



knotGrid <- knotGrid %>% orderBy(~Bm.index + Bl.index,.)
...
B. <- Bl.*Bm.
#basisKeepIndex <- knotGrid$keep[!duplicated(knotGrid[,c("Bl.index","Bm.index")])] %>% equals(TRUE) %>% which
...
#reference_grid <- data.frame(l=l.,m=m.)
...
test_X <- matrix(data=0,nrow=m-1,ncol=nrow(reference_grid))
for(row.i in 1:nrow(grid)){
      X[grid$t[row.i]-1,grid$column_index[row.i]] <- as.vector(y)[grid$s[row.i]] 
      test_X[grid$t[row.i]-1,grid$column_index[row.i]] <- as.vector(1:m)[grid$s[row.i]] 
}
U. <- X%*%B.
