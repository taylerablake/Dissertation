...
psp2dM<-function(y, M, p1, p2, M.type = "stacked", M1.index = NULL, M2.index = NULL, 
                 Pars, ridge.adj = 0, M.pred = M, y.predicted = NULL, x.lab = "X1", 
                 y.lab = "X2", z.lab = "A.hat", coef.plot = T,
                 image.plot = T, se.bands=T, family = "gaussian",
                 link = "default", m.binomial = NULL,
                 wts = NULL, r.gamma = NULL, int = F){
      ...
      if(M.type == "stacked") {
            ...
            p2. <- ncol(M)
            if(p1 != p1. | p2 != p2.) {
                  warning(paste("recheck input p1 or p2: at least one dimension does not match"))
            }
            x <- as.vector(t(M))
      ...
      if(coef.plot) {
            ...
            if(length(M1.index) > 100) {
                  i.1 <- round(seq(from = min(M1.index), to = max(M1.index), length = 100))
                  in1 <- round(seq(from = 1, to = p1, length = 50))
            }
            if(length(M2.index) > 100) {
                  i.2 <- round(seq(from = min(M2.index), to = max(M2.index), length = 100))
                  in2 <- round(seq(from = 1, to = p2, length = 50))
            }
            persp(M2.index[in2], (M1.index[in1]), t(A.hatm[in1, in2]),
                  xlab = y.lab, ylab = x.lab, zlab = z.lab)
            if(image.plot) {
            ...
            if(se.bands) {
                  ...
                  persp(M2.index[in2], M1.index[in1], t(U.hatm[in1, in2]),
                        xlab = x.lab, ylab = y.lab, zlab =  "2 se Upper Surface")
                  if(image.plot) {
                        image(M2.index[in2], M1.index[in1], t(U.hatm[in1, in2]),
                              xlab = x.lab, ylab = y.lab, sub = "2 se Upper Surface")
                  }
                  persp(M2.index[in2], M1.index[in1], t(L.hatm[in1, in2]),
                        xlab = x.lab, ylab = y.lab, zlab = "2 se Lower Surface")
                  if(image.plot) {
                        image(M2.index[in2], M1.index[in1], t(L.hatm[
                              in1, in2]), xlab = x.lab, ylab = y.lab, sub = "2 se Lower Surface")
                  }
            }
      }
      ...
      P <- list(coef = pcoef,
                ...
                bin.1 = bin.1percent,
                df.resid = m - trace,
                dispersion.parm = dispersion.parm, 
                mu = mu,
                press.mu = press.mu,
